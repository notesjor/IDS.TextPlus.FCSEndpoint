...
public static class LexCqlParser
{
  ...
  private static readonly TokenListParser<LexCqlToken, RelationModifier> Modifier =
    ...
    .OptionalOrDefault(RelationModifier.None);
  ...
  private static readonly TokenListParser<LexCqlToken, LexCqlExpression> ImplicitComparison =
    Value.Select(v => (LexCqlExpression)new ComparisonExpression(DefaultField, LexCql.RelationOp.Equals, RelationModifier.None, v));
  ...
  private static RelationModifier ParseModifier(string name) => name switch
  {
  ...
  /// </summary>
  private static string UnescapeBare(string raw) => Unescape(raw);
