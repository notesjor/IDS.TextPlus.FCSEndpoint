...
public abstract class AbstractVersion
{
    ...
    string nameSpec, int defaultValue, int minValue, out int returnValue, string template)
  {
    ...
    if (data.ContainsKey(name))
    {
      ...
      {
        ctx.Response.WriteAsync(template.Replace("{{name}}", nameSpec).Replace("{{message}}", "Invalid number format."));
        return true;
      }

      if (returnValue < minValue)
      {
        ctx.Response.WriteAsync(template.Replace("{{name}}", nameSpec).Replace("{{message}}", $"Value is less than {minValue}."));
        return true;
      }
    }
    ...
    string nameSpec, int defaultValue, int minValue, int maxValue, out int returnValue, string template)
  {
    ...
    if (data.ContainsKey(name))
    {
      if (!int.TryParse(data[name], out returnValue))
      {
        ctx.Response.WriteAsync(template.Replace("{{name}}", nameSpec).Replace("{{message}}", "Invalid number format.")).Wait();
        return true;
      }

      if (returnValue < minValue)
      {
        ctx.Response.WriteAsync(template.Replace("{{name}}", nameSpec).Replace("{{message}}", $"Value is less than {minValue}.")).Wait();
        return true;
      }

      if (returnValue > maxValue)
      {
        ctx.Response.WriteAsync(template.Replace("{{name}}", nameSpec).Replace("{{message}}", $"Value is greater than {maxValue}.")).Wait();
        return true;
      }
    }
